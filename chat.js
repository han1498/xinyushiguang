...
Page({
      ...
      .then((reply) => {
        ...
        if (reminders.length > 0) {
          ...
          if (added.length > 0) {
            ...
            this._syncRemindersToTodos(added)
          }
        }
      })
      .catch(() => {
  ...
  _syncEventsToTodos(events) {
    const todos = safeGetStorage(STORAGE_KEYS.TODO_DATA, [])
    const added = []
    for (const ev of events) {
      const content = ev.title || ""
      const date = ev.date || getTodayYMD()
      const exists = todos.some((t) => t.content === content && t.date === date)
      if (!exists && content) {
        added.push({
          id: generateId("t"),
          date,
    ...
    if (added.length > 0) {
      safeSetStorage(STORAGE_KEYS.TODO_DATA, todos.concat(added))
    }
  },

  _syncRemindersToTodos(reminders) {
    const todos = safeGetStorage(STORAGE_KEYS.TODO_DATA, [])
    const added = []
    for (const r of reminders) {
      ...
      let expireDate = ""
      if (r.dateTime && typeof r.dateTime === "string" && r.dateTime.length >= 10) {
      ...
      if (!exists && content) {
        added.push({
          ...
          createdAt: Date.now(),
          duration: expireDate ? "custom" : "permanent",
          expireDate
        })
      }
    }
